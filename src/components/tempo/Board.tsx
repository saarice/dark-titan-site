...
export default function Board() {
  ...
  useEffect(() => {
    ...
    // SPEED < 1 plays the replay slower than real time so each ticket's journey
    // across the lanes is readable instead of a blur.
    const SPEED = 0.5;
    ...
    const frame = (now: number) => {
      ...
      setTms((now - start) * SPEED);
      raf = requestAnimationFrame(frame);
    };

    // Don't run from page load — start the clock only once the board scrolls
    // into view, so you catch the replay from a fresh (already-populated) frame
    // when you reach it rather than arriving mid-stream.
