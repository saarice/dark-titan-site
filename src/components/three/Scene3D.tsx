...
export type MonolithVersion = "d" | "e";
  ...
  return (
    <div className="fixed inset-0" style={{ zIndex: 0, pointerEvents: "none" }} aria-hidden>
      ...
      >
        ...
        <fog attach="fog" args={["#0A0A0C", cinematic ? 8 : 9, cinematic ? 30 : 26]} />
        <ambientLight intensity={cinematic ? 0.32 : 0.4} />
        ...
        <pointLight position={[0, 1.5, -6]} intensity={cinematic ? 16 : 12} color="#9B6DFF" distance={28} />

        {/* D only: stormy cloud bank behind + violet ground glints */}
        {cinematic && !reduced && (
          <>
        ...
        <MonolithShards
          variant={cinematic ? "obelisk" : "slabs"}
          scroll={scroll}
          ptr={ptr}
          reduced={reduced}
        />
        ...
        <EffectComposer>
          <Bloom
            intensity={reduced ? 0.4 : isMobile ? 0.7 : cinematic ? 1.2 : 0.95}
            luminanceThreshold={cinematic ? 0.4 : 0.45}
            luminanceSmoothing={0.9}
            mipmapBlur
            radius={cinematic ? 0.8 : 0.7}
          />
        </EffectComposer>
      </Canvas>
    </div>
